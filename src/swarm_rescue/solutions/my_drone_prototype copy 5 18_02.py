...
# --- DRAW BREADCRUMBS (Hansel & Gretel trail) ---
 
        if hasattr(self, 'breadcrumbs') and len(self.breadcrumbs) > 1:
            for i, (bx, by) in enumerate(self.breadcrumbs):
                pt = np.array([bx, by]) + self._half_size_array
                
                # Draw crumb dot - orange color
                arcade.draw_circle_filled(pt[0], pt[1], radius=5, color=(255, 140, 0))
