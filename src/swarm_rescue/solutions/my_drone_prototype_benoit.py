...
# --- MODIFIED: Keep multiple rescue zone points instead of merging ---
    ...
    def update_pose(self):
        ...
        if gps_pos is not None and not np.isnan(gps_pos[0]):
            ...
            # --- CORRECTION ICI : SÉCURITÉ COMPAS ---
            # On ne prend le compas que s'il est valide
            if compass_angle is not None:
                self.current_pose[2] = compass_angle
            # Sinon, on garde l'ancien angle (ou on pourrait ajouter l'odométrie ici aussi)

        # --- CAS 2 : GPS PERDU (Odométrie + Prédiction) ---
        else:
            ...
            # Mise à jour de l'orientation (Toujours valide car vient de l'odométrie interne)
            # Attention: s'assurer que self.current_pose[2] n'est pas None au départ
            if self.current_pose[2] is None:
                self.current_pose[2] = 0.0 # Valeur par défaut si tout échoue au début
            
            self.current_pose[2] += rotation_change
            self.current_pose[2] = normalize_angle(self.current_pose[2])
            
            if self.kf_initialized:
                # Prédiction Kalman pure (Attention à la dérive de vitesse !)
                F = np.array([
                    [1, 0, self.kf_dt, 0],
                    [0, 1, 0, self.kf_dt],
                    [0, 0, 1, 0],
        ...
        # # --- ÉTAPE FINALE CRUCIALE ---
        # # Il faut transférer les calculs (current_pose) vers l'objet utilisé par le reste du code (estimated_pose)
        # from swarm_rescue.utils import Pose # Assurez-vous d'avoir l'import
        # self.estimated_pose = Pose(self.current_pose[0], self.current_pose[1], self.current_pose[2])
